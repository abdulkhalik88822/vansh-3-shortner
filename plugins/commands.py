...
async def start(client:Client, message): 
    ...
    if settings.get('fsub_id', AUTH_CHANNEL) and not await is_subscribed(client, message.from_user.id, channel):
        invite_link = "https://t.me/+dNFeS1Lr_-tlYWY1"
        btn = [[
...
async def tutorial_three(bot, message):
    ...
    try:
        ...
        return await message.reply_text("<b><u><i>ɪɴᴠᴀɪʟᴅ ꜰᴏʀᴍᴀᴛ!!</i></u>\n\nᴜsᴇ ʟɪᴋᴇ ᴛʜɪs - </b>\n<code>/tutorial3 https://youtu.be/</code>")
    reply = await message.reply_text("<b>ᴘʟᴇᴀꜱᴇ ᴡᴀɪᴛ...</b>")
...
async def set_shortner(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.me/none_090').json()
...
async def set_shortner_3(c, m):
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://youtube.com/').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
...
async def deletemultiplefiles(bot, message):
    ...
    btn = [[
       InlineKeyboardButton("Yᴇs, Cᴏɴᴛɪɴᴜᴇ !", callback_data=f"rahulreviewsyt#{keyword}")
       ],[
